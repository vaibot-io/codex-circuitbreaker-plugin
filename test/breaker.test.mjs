...
// Each breaker test gets its own fake HOME so breaker state at
// ~/.vaibot/breaker-state/codex.json doesn't leak between tests or pollute
// the user's real ~/.vaibot.
function withFakeHome(fn) {
  return async (t) => {
    const home = mkdtempSync(join(tmpdir(), 'vaibot-breaker-'))
    try {
      await fn(t, home)
    } finally {
...
function runHook({ apiUrl, mode = 'enforce', input, env = {}, home }) {
  // Sandbox $TMPDIR/vaibot-codex/ under the per-test fakeHome so STATE_DIR is
  // isolated from other test files running in parallel under node:test.
...
test('three consecutive 5xx trip the breaker; next non-allowlisted call denies locally without hitting API', withFakeHome(async (t, home) => {
  ...
  for (let i = 0; i < 3; i++) {
    await runHook({
      apiUrl: server.url,
      input: { ...baseInput, tool_input: { command: `attempt ${i}` } },
      home,
    })
  }
...
test('denylist denies even with API healthy when breaker is tripped', withFakeHome(async (t, home) => {
  // Trip the breaker first via 5xx server.
  ...
  // Even with a healthy server, the breaker (still tripped) takes over.
...
test('401 (auth) does NOT count as a transient failure', withFakeHome(async (t, home) => {
  ...
  // After 5 auth failures, breaker should still be CLOSED — state file either
  // missing or shows failures: [].
...
test('successful API call resets the failure window (closes after partial failures)', withFakeHome(async (t, home) => {
  // 2 failures (below threshold of 3), then 1 success — failures should reset.
  let calls = 0
...
test('cooldown auto-resets the breaker after the window elapses', withFakeHome(async (t, home) => {
  // Use a tiny cooldown so the test can wait for it.
  const trip = await startMockServer(() => ({ status: 500, body: { error: 'oops' } }))
  for (let i = 0; i < 3; i++) {
    await runHook({
      apiUrl: trip.url,
  ...
  // Next call with healthy server should pass — breaker auto-resets on
  // isTripped() read, and the success then clears the failure window.
  const healthy = await startMockServer(() => ({
    status: 200,
    body: {
  ...
  assert.equal(r.code, 0)
  assert.equal(r.stdout.trim(), '', 'API allowed → empty stdout')
...
test('observe mode + tripped breaker logs breadcrumb and allows (no block)', withFakeHome(async (t, home) => {
  ...
  assert.equal(r.code, 0)
  assert.equal(r.stdout.trim(), '', 'observe must not block')
